...
for i = 1:4
pal = PalSrc('audio_freq', audio_freq(i), 'ultra_freq', 40e3, 'src', src);
...
prs(i) = PalCircSrc_SWE(pal, fp, 'la_max', 40);

[prs_low, vel_low] = CircPiston_Direct_Tao(pal.ultra_low.num, 0.1, ...
    fp.x, fp.y, fp.z,'profile_type','uniform');
[prs_high, vel_high] = CircPiston_Direct_Tao(pal.ultra_high.num, 0.1, ...
    fp.x, fp.y, fp.z,'profile_type','uniform');

lag(i) = 1.21/2 .* (vel_high.x .* conj(vel_low.x) ...
