...
% =========================================================================
function [prs, vel] = CircPal_SWE_Addition_new(pal1, pal2, fp, nuMax, T1, T2, Gaunt_coeff, br, varargin)
    ...
    nu1 = vec2ndim(0:nuMax,1);
    mu1 = vec2ndim(-nuMax:nuMax,2);
    nu2 = vec2ndim(0:nuMax,3);
    mu2 = vec2ndim(-nuMax:nuMax,4);
    
    la = vec2ndim(0:(4*nuMax+1), 5);
    ...
    if fp.r < br
        ...
        R_temp = CircPal_SWE_Addition_Radial_int_dim(pal_radius, br, la, wn1, fp.r, nu1, nu2);
        ...
        R_temp = CircPal_SWE_Addition_Radial_ext_new_0217(pal_radius, br, la, wn1, fp.r, nu1, nu2);
        coeff_rad1 = Tcoeff.int;
    ...
    %% Spherical harmonics
    % Y dim: 1 -> mu1 -> 1 -> mu2 -> la

    Y = SphHarmonic_ext_new(la, mu1, mu2, fp.theta, fp.phi);
    
    %% Caculate audio sound at one field point
    
    p0 = 1.21*343*1;
    % Gaunt coeff dim: nu1 -> mu1 -> nu2 -> mu2 -> la
    prs = 16 * pi^2 * 1.2 * p0^2 / (1i*1.21*343^2) * ... 
        sum((-1).^mu2 .* Gaunt_coeff .* R .* Y, 'all');
