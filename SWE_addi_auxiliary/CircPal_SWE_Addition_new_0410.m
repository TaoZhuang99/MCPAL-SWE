...
% =========================================================================
function [prs, vel] = CircPal_SWE_Addition_new_0410(pal1, pal2, fp, Y_tot, sum_coef, T1, T2, Gaunt_coeff, br, varargin)
...
for ir = 1:length(fp.r)
    ...
    %% Spherical harmonics
    % Y_tot dim: nu1 -> mu1 -> nu2 -> mu2 -> la -> theta -> phi
    % Gaunt coeff dim: nu1 -> mu1 -> nu2 -> mu2 -> la

    prs(ir, :, :) = 16 * pi^2 * 1.2 * p0^2 / (1i*1.21*343^2) * ... 
    permute(sum((-1).^sum_coef.mu2 .* Gaunt_coeff .* R .* Y_tot, [1 2 3 4 5]), [5 6 7 1 2 3 4]);
