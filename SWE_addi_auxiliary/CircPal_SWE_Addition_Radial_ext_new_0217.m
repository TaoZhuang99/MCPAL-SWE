...
function res = CircPal_SWE_Addition_Radial_ext_new_0217(pal_radius, br, la, wn, r, nu1, nu2)
    ...
    % integral range
    range.int1 = 1e-6; range.int2 = br-pal_radius;
    range.ext2 = rvMax;

    if r <= br+pal_radius
        range.mid1 = 0; range.mid2 = 0;
        range.ext1 = br+pal_radius;
    else 
        range.mid1 = br+pal_radius; range.mid2 = r;
        range.ext1 = r;
    end
	...
	func_mid = SphBesselJ(la, ka*rv_mid) .* SphHankelH(la, ka*r) .* ...
		SphHankelH(nu1, k1*rv_mid) .* conj(SphHankelH(nu2, k2*rv_mid)) .* ...
		ka^3 .* rv_mid.^2;
